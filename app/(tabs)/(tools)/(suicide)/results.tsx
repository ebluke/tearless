...
import { getResultCards } from "@/lib/results/suicideResults";
...
const SuicideResults = () => {
  ...
  const clear = () => {
    ...
    suicideStore.clearAnswers();
    router.back();
  };

  const save = () => {
    suicideStore.saveResults();
    clear();
  };
  ...
  const styles = StyleSheet.create({
    ...
    miniScore: {
      borderRadius: 20,
      backgroundColor:
        suicideStore.totalScore < 5
          ? Colors[theme].green
          : suicideStore.totalScore < 15
          ? Colors[theme].orange
          : Colors[theme].red,
      width: 20,
      height: 15,
      ...
      <View style={styles.scores}>
        <View style={styles.scoreTop}>
          <ThemedText type="defaultSemiBold">
            Suicide Screening Score: {suicideStore.totalScore}
          </ThemedText>
      ...
      <View>
        {results.map((item, key) => (
          <TResultsCard
            ...
            key={key}
            result={item}
            totalScore={suicideStore.totalScore}
          />
        ))}
      </View>
