...
export default function AnxietyGuide() {
  ...
  const handleStart = () => {
    router.push("/(tabs)/(tools)/(anxiety)/anxiety");
  };

  return (
    <TabWrapper>
      <ThemedText type="title">Anxiety Screening</ThemedText>
      <Card
        header="Tool Usage:"
        body={
          "The following screening tool is used to help screen individuals for anxiety. \n\nPlease read the prompt to the individual prior to the screening process."
        }
      />
      <Card
      ...
      <Button
        title="Continue"
        onPress={() => router.push("/(tabs)/(tools)/(anxiety)/anxiety")}
      />
    </TabWrapper>
  );
}
